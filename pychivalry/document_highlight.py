...
SYMBOL_PATTERNS = {
    ...
    "variable_set": re.compile(
        r"\bset_(?:local_|global_)?variable\s*=\s*\{[^}]*name\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*)"
    ),
    ...
    "character_modifier": re.compile(
        r"\b(?:add_|remove_|has_)character_modifier\s*=\s*(?:\{[^}]*modifier\s*=\s*)?([a-zA-Z_][a-zA-Z0-9_]*)"
    ),
    # Trait references
...
def _get_patterns_for_symbol(
    symbol: SymbolInfo,
) -> List[Tuple[re.Pattern, types.DocumentHighlightKind]]:
    """
    ...
    if symbol.symbol_type in ("scope_reference", "scope_definition"):
        ...
        patterns.append(
            (
                re.compile(
                    rf"\bchange_(?:local_|global_)?variable\s*=\s*\{{[^}}]*name\s*=\s*{name}"
                ),
                types.DocumentHighlightKind.Write,
            )
        )
