...
class DocumentIndex:
    ...
    def __init__(self):
        ...
        self.on_action_definitions: Dict[str, types.Location] = {}  # name -> Location (actual definitions)
    ...
    def _scan_workspace_sequential(self, workspace_roots: List[str]):
        ...
        for root in workspace_roots:
            ...
            if interactions_path.exists() and interactions_path.is_dir():
                self._scan_common_folder(interactions_path, self.character_interactions, "character interaction")
    ...
    def _scan_common_folder(self, folder_path: Path, target_dict: Dict[str, types.Location], def_type: str):
        """
    ...
    def _scan_events_folder(self, folder_path: Path):
        ...
        for file_path in folder_path.glob("**/*.txt"):
            try:
                ...
                logger.debug(f"Indexed {len(namespaces)} namespaces, {len(events)} events, {len(scopes)} scopes from {file_path.name}")
        ...
        for line_num, line in enumerate(lines):
            ...
            if current_depth == 0:
                ...
                if match:
                    ...
                    if name not in ('if', 'else', 'else_if', 'trigger', 'effect', 'limit', 'modifier',
                                   'hidden_effect', 'show_as_tooltip', 'random_list', 'switch'):
    ...
    def _remove_document_entries(self, uri: str):
        ...
        self.character_interactions = {k: v for k, v in self.character_interactions.items() if v.uri != uri}
        ...
        self.on_action_definitions = {k: v for k, v in self.on_action_definitions.items() if v.uri != uri}
