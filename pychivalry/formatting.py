...
class CK3Formatter:
    ...
    def _format_lines(self, lines: List[str], start_indent: int, line_count: int) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if (indent_level == 0 and prev_was_block_end and 
                formatted and formatted[-1] != '' and
                self.options.blank_lines_between_blocks > 0):
                formatted.append('')
...
def format_document(text: str, options: Optional[types.FormattingOptions] = None) -> List[types.TextEdit]:
    """
...
def format_range(
    text: str, 
    range_: types.Range,
    options: Optional[types.FormattingOptions] = None
) -> List[types.TextEdit]:
    """
    ...
    try:
        ...
        formatted_text, actual_start, actual_end = formatter.format_range(
            text, 
            range_.start.line, 
            range_.end.line + 1  # Convert to exclusive
        )
