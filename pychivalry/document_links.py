...
LINKABLE_EXTENSIONS = (
    '.txt', '.yml', '.yaml', '.dds', '.png', '.tga', '.mesh', '.asset',
    '.gui', '.gfx', '.shader', '.settings', '.mod', '.json',
)
...
GFX_SCRIPT_PATTERN = re.compile(
    r'(?:icon|texture|sprite|background)\s*=\s*"([^"]+)"'
)
...
def _find_mod_root(start_dir: str) -> Optional[str]:
    ...
    for _ in range(10):
        ...
        if (os.path.isdir(os.path.join(current, 'common')) or
            os.path.isdir(os.path.join(current, 'events'))):
            return current
