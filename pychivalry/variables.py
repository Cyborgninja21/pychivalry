...
def validate_change_variable(params: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if not has_operation:
        return (False, "change_variable requires at least one operation (add, subtract, multiply, divide)")
...
def validate_clamp_variable(params: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if 'min' in params and 'max' in params:
        try:
            ...
            if min_val > max_val:
                return (False, f"clamp_variable min ({min_val}) cannot be greater than max ({max_val})")
...
def validate_variable_list_operation(operation: str, params: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """
...
def create_variable(name: str, scope: str = 'var', value: Any = None, is_list: bool = False) -> Variable:
    """
