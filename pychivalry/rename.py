...
RENAME_PATTERNS = {
    ...
    'variable': {
        ...
        'definition': re.compile(r'\bset_(?:local_|global_)?variable\s*=\s*\{[^}]*name\s*=\s*({name})'),
        # Reference: var:counter
        'reference': re.compile(r'\b(?:local_|global_)?var:({name})\b'),
        # Change: change_variable = { name = counter ... }
        'change': re.compile(r'\bchange_(?:local_|global_)?variable\s*=\s*\{[^}]*name\s*=\s*({name})'),
    },
    'character_flag': {
        # Add flag (definition)
...
def get_symbol_at_position(
    ...
    scope_def_match = re.search(r'\bsave_(?:temporary_)?scope_as\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*)', line)
    ...
    flag_match = re.search(r'\b(?:has_|add_|remove_)character_flag\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*)', line)
    ...
    global_flag_match = re.search(r'\b(?:has_|set_|remove_)global_flag\s*=\s*([a-zA-Z_][a-zA-Z0-9_]*)', line)
        ...
        for scan_pattern in scan_patterns:
            ...
            if os.path.exists(folder_path):
                _scan_folder_for_symbol(
                    folder_path, symbol_name, symbol_type,
                    all_locations, processed_uris
                )
...
def _get_scan_patterns_for_type(symbol_type: str) -> List[str]:
    ...
    if symbol_type == 'event':
        ...
        return ['events', 'common/scripted_effects', 'common/scripted_triggers', 'common/on_actions']
    else:
                    ...
                    for line_num, line in enumerate(lines):
                        ...
                        if match:
                            ...
                            locations.append(RenameLocation(
                                ...
                                range=types.Range(
                                    start=types.Position(line=line_num, character=start_char),
                                    end=types.Position(line=line_num, character=start_char + len(key)),
                                ),
                                old_text=key,
