...
IMPORTANT_BLOCKS = {
    # Event structure
    'trigger', 'immediate', 'option', 'after', 'desc',
    # Common blocks
    'effect', 'modifier', 'limit', 'if', 'else', 'else_if',
    # Iterators
    'every_', 'random_', 'any_', 'ordered_',
    # Lists
    'OR', 'AND', 'NOT', 'NOR', 'NAND',
    # Scripted content
    'scripted_trigger', 'scripted_effect',
}
...
def _get_comment_folding_ranges(text: str) -> List[types.FoldingRange]:
    ...
    for line_num, line in enumerate(lines):
        stripped = line.strip()
        is_comment = stripped.startswith('#') and not stripped.startswith('# region') and not stripped.startswith('# endregion')
