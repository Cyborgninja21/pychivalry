...
def tokenize(text: str) -> List[CK3Token]:
    ...
    for line_num, line in enumerate(lines):
        ...
        while col < len(line):
            ...
            if line[col].isdigit() or (line[col] == '-' and col + 1 < len(line) and line[col + 1].isdigit()):
                num_end = col + 1
                has_decimal = False
            ...
            if value is not None:
                ...
                node = CK3Node(
                    ...
                    range=types.Range(
                        ...
                        end=types.Position(line=key_token.line, character=key_token.character + len(key_token.value)),
                    ),
                    parent=parent,
                )
                return node
