...
def get_code_lenses(
    ...
    if 'scripted_effects' in document_uri:
        effect_lenses = _find_scripted_effect_lenses(lines, document_uri, document_index, processed_symbols)
    ...
    if 'scripted_triggers' in document_uri:
        trigger_lenses = _find_scripted_trigger_lenses(lines, document_uri, document_index, processed_symbols)
...
def _find_event_lenses(
    ...
    for line_num, line in enumerate(lines):
        ...
        if match:
            ...
            ref_count, trigger_event_count, missing_loc = _analyze_event(
                event_id, document_index
            )
...
def _find_scripted_effect_lenses(
    ...
    skip_keywords = {'if', 'else', 'else_if', 'trigger', 'effect', 'limit', 
                     'modifier', 'hidden_effect', 'show_as_tooltip', 'random_list', 'switch'}
