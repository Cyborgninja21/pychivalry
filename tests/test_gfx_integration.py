...
class TestGFXIntegration:
    ...
    def test_gfx_validation_with_example_mod(self):
        ...
        diagnostics = collect_all_diagnostics(
            doc, ast, index=None, workspace_folders=workspace_folders
        )
        ...
        assert (
            len(gfx_diagnostics) >= 2
        ), f"Expected at least 2 GFX001 diagnostics, got {len(gfx_diagnostics)}"
    ...
    def test_no_false_positives_on_valid_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            diagnostics = collect_all_diagnostics(
                doc, ast, index=None, workspace_folders=[str(tmp_path)]
            )
    ...
    def test_gfx_validation_disabled(self):
        ...
        diagnostics = collect_all_diagnostics(
            doc, ast, index=None, config=config, workspace_folders=[]
        )
