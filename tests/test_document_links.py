...
class TestFilePathDetection:
    ...
    def test_common_folder_path(self):
        ...
        assert len(links) >= 1
        assert any(
            "common/scripted_effects/my_effects.txt" in (link.tooltip or "") for link in links
        )
    ...
    def test_events_folder_path(self):
        ...
        assert len(links) >= 1
        path_found = any(
            "events/my_mod/chapter_one.txt" in str(link.range)
            or "events/my_mod/chapter_one.txt" in (link.tooltip or "")
            for link in links
        )
